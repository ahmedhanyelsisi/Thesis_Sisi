...
%% ========================================================================
%  4a_CORR. PHASE ALIGNMENT & POLARITY CORRECTION (NEW)
%  ========================================================================
%  Auto-detects timing offset between voltage and current via cross-correlation.
%  If the offset is >90°, it likely means the current probe was connected
%  with reversed polarity. The user is prompted to invert it.
%  The residual timing offset is then applied to align the signals.
%  ========================================================================

% Pre-correction PF calculation
% Compute uncorrected PF over the overlapping window for comparison
tCommon_raw = t_v((t_v >= overlapStart) & (t_v <= overlapEnd));
Va_raw = V((t_v >= overlapStart) & (t_v <= overlapEnd), 1);
Vb_raw = V((t_v >= overlapStart) & (t_v <= overlapEnd), 2);
Vc_raw = V((t_v >= overlapStart) & (t_v <= overlapEnd), 3);

Ia_raw_interp = interp1(t_c_dec, I_dec(:,1), tCommon_raw, 'linear', 'extrap');
Ib_raw_interp = interp1(t_c_dec, I_dec(:,2), tCommon_raw, 'linear', 'extrap');
Ic_raw_interp = interp1(t_c_dec, I_dec(:,3), tCommon_raw, 'linear', 'extrap');

P_total_raw = mean(Va_raw.*Ia_raw_interp) + mean(Vb_raw.*Ib_raw_interp) + mean(Vc_raw.*Ic_raw_interp);
S_total_raw = sqrt(mean(Va_raw.^2))*sqrt(mean(Ia_raw_interp.^2)) + ...
              sqrt(mean(Vb_raw.^2))*sqrt(mean(Ib_raw_interp.^2)) + ...
              sqrt(mean(Vc_raw.^2))*sqrt(mean(Ic_raw_interp.^2));
PF_before = P_total_raw / S_total_raw;

% Use Phase A only (representative since offset is identical across phases)
% Search window: +/- 20 ms around zero lag
maxLag_samples = round(0.020 / tInc_c_dec);  % +/- 20 ms in samples
[r, lags] = xcorr(V(:,1), I_dec(:,1), maxLag_samples);
[~, maxIdx] = max(r);
sampleShift_raw = lags(maxIdx);
timeShift_raw_ms = sampleShift_raw * tInc_c_dec * 1e3;
phaseShift_raw_deg = (timeShift_raw_ms / 1e3) * 50 * 360;

polarityInverted = false;   % robust flag (avoids fragile 'exist' checks)

if abs(phaseShift_raw_deg) > 90
    % Ask user via questdlg
    resp = questdlg(sprintf([...
        'Cross-correlation detected a %.1f° (%.3f ms) phase shift between\n' ...
        'voltage and current. This is greater than 90°, which strongly\n' ...
        'suggests the current probes were connected with REVERSED POLARITY.\n\n' ...
        'Invert current polarity (multiply by -1)?'], ...
        phaseShift_raw_deg, timeShift_raw_ms), ...
        'Polarity Reversal Detected', 'Yes (Recommended)', 'No (Pure Time Shift)', 'Yes (Recommended)');
    
    if strcmp(resp, 'Yes (Recommended)')
        polarityInverted = true;
        I_dec = -I_dec;
        I_raw = -I_raw;
        fprintf('  Current polarity INVERTED (multiplied by -1).\n');
        
        % Re-run cross-correlation AFTER inversion to find the TRUE residual timing offset
        [r2, lags2] = xcorr(V(:,1), I_dec(:,1), maxLag_samples);
        [~, maxIdx2] = max(r2);
        sampleShift = lags2(maxIdx2);
        timeShift_ms = sampleShift * tInc_c_dec * 1e3;
        phaseShift_deg = (timeShift_ms / 1e3) * 50 * 360;
    else
        sampleShift = sampleShift_raw;
        timeShift_ms = timeShift_raw_ms;
        phaseShift_deg = phaseShift_raw_deg;
    end
else
    sampleShift = sampleShift_raw;
    timeShift_ms = timeShift_raw_ms;
...
% --- Apply timing correction by shifting the CURRENT TIME VECTOR ---
% Instead of physically moving data samples and padding (which creates DC
% steps that cause severe filtfilt ringing), we shift t_c_dec so the
% current waveform is re-timed relative to the voltage.  The overlap
% window is then recomputed to cover only the region where BOTH signals
% have real data -- no junk/padded samples enter the analysis.
timeShift_s = timeShift_ms / 1e3;
t_c_dec = t_c_dec - timeShift_s;

% Recompute overlap window: only the region where both records have data
overlapStart = max(0, t_c_dec(1));
overlapEnd   = min(t_v(end), t_c_dec(end));

if overlapEnd <= overlapStart
    error(['No usable synchronized data after phase correction. ' ...
           'The time shift (%.3f ms) may be too large.'], timeShift_ms);
end

fprintf('\nPhase Alignment Summary:\n');
fprintf('  Raw cross-correlation offset : %.3f ms (%.1f°)\n', timeShift_raw_ms, phaseShift_raw_deg);
if polarityInverted
    fprintf('  After polarity inversion     : %.3f ms (%.1f°)\n', timeShift_ms, phaseShift_deg);
end
fprintf('  Applied time shift            : %.3f ms\n', timeShift_ms);
fprintf('  Polarity inverted             : %s\n', iif(polarityInverted, 'YES', 'NO'));
fprintf('  PF before correction          : %.4f\n', PF_before);
fprintf('  Updated overlap window        : [%.3f, %.3f] ms  (%.3f ms)\n\n', ...
        overlapStart*1e3, overlapEnd*1e3, (overlapEnd-overlapStart)*1e3);
...
titleStr = 'Synchronized Grid Voltage & Current (t=0 to end of current log, Phase Corrected): Raw (light) vs. Filtered (bold)';
if hasLoadStep
